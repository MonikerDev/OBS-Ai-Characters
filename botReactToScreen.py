...
import ntpath
import YosukeBrain as Yosuke
import FuukaBrain as Fuuka
...
def script_load(settings):
    ...
    hotkey_id_array.append(obs.obs_hotkey_register_frontend("READSCREEN", "Yosuketalk", hotkey_1_callback))
    hotkey_names_by_id[hotkey_id_array[len(hotkey_id_array)-1]] = "READSCREEN"
...
def hotkey_1_callback(is_pressed):
    if(is_pressed):
        ...
        save_path = r"C:\Users\PC\Desktop\YosukeBot\Screenshot.png"
        snapshot.save(save_path)
        image = PIL.Image.open(save_path)
        Yosuke.respondToImage(image)
        enable_source(False)
